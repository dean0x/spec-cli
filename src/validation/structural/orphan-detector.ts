 ...
 */
export function isIndexOrEntryPoint(filePath: string): boolean {
...
export function findOrphans(
  ...
  for (const [filePath] of files) {
    ...
    if (isIndexOrEntryPoint(filePath)) continue;
 ...
 */
export function generateOrphanStats(
  files: Map<string, string>
): { total: number; orphans: number; referenced: number } {
  const referenced = buildReferenceSet(files);
  let total = 0;
  let orphanCount = 0;
