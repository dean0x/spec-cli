 ...
 */
export function validateStructure(files, options = DEFAULT_OPTIONS) {
    const allIssues = [];
    ...
    if (options.checkFrontmatter !== false) {
        for (const [filePath, content] of files) {
            ...
            if (componentType) {
                ...
                const typeKey = Object.entries(COMPONENT_TYPES).find(([, v]) => v === componentType)?.[0];
                if (typeKey) {
 ...
 */
export function formatValidationResultJson(result) {
