 ...
 */
export function getTemplateVariables(componentType) {
...
export function applyTemplate(template, context) {
    ...
    for (const varName of variables) {
        if (!(varName in context) && !COMMON_VARIABLES.find(v => v.name === varName && !v.required)) {
            ...
            const isRequired = Object.values(TYPE_VARIABLES).some(vars => vars.find(v => v.name === varName && v.required));
            if (isRequired) {
 ...
 */
export function getComponentPath(componentType, name, parentContext) {
