...
export function parseManifest(content) {
    ...
    for (const line of lines) {
        ...
        if (colonIndex > 0 && !trimmed.startsWith('-')) {
            ...
            if (currentSection === 'root') {
                ...
                if (value.startsWith('[') && value.endsWith(']')) {
                    ...
                    if (currentSection === 'owns') {
                        manifest.owns[key] = items;
                    }
                    else {
                        manifest.uses[key] = items;
                    }
                    currentKey = null;
                }
                else if (!value) {
            ...
            if (currentSection === 'owns') {
                manifest.owns[currentKey]?.push(item);
            }
            else {
                manifest.uses[currentKey]?.push(item);
            }
        }
    }
...
export function serializeManifest(manifest) {
    ...
    // Owns section
    const ownsEntries = Object.entries(manifest.owns).filter(([, v]) => Array.isArray(v) && v.length > 0);
    ...
    // Uses section
    const usesEntries = Object.entries(manifest.uses).filter(([, v]) => Array.isArray(v) && v.length > 0);
 ...
 */
export function validateManifestOwnership(manifest, existingFiles) {
    const issues = [];
    ...
    const ownsEntries = Object.entries(manifest.owns);
 ...
 */
export function getOwnedFilePaths(manifest) {
    const paths = [];
    const typeToDir = {
        schemas: 'docs/schemas',
        domains: 'docs/domains',
 ...
 */
export function isFileOwnedByManifest(filePath, manifest) {
 ...
 */
export function findReferencingFeatures(manifest, allManifests, graph) {
    const referencingFeatures = new Set();
    const ownedPaths = getOwnedFilePaths(manifest);
    // For each file in the graph, check if it references our owned files
    for (const [filePath, node] of graph.nodes) {
        // Skip our own files
        if (isFileOwnedByManifest(filePath, manifest))
            continue;
        ...
        for (const linkedTo of node.linksTo) {
            ...
            for (const ownedPath of ownedPaths) {
                const matches = linkedToNormalized === ownedPath ||
 ...
 */
export function computeReferencedBy(manifests, graph) {
 ...
 */
export function updateManifestReferencedBy(manifest, referencedBy) {
    return {
        ...manifest,
        referencedBy,
    };
 ...
 */
export function isReferencedByStale(manifest, computedRefs) {
    const currentRefs = manifest.referencedBy ?? [];
    if (currentRefs.length !== computedRefs.length)
        return true;
