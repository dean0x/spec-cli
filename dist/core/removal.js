 ...
 */
export function createRemovalMarker(linkText, linkPath, featureName) {
    return {
 ...
 */
export function getOwnedFilesFromGraph(manifest, graph) {
 ...
 */
export function findBreakingEdges(filesToRemove, graph) {
    const removeSet = new Set(filesToRemove);
    return graph.edges.filter((edge) => removeSet.has(edge.to) && !removeSet.has(edge.from));
}
/**
 * Convert breaking edges to breaking references with markers
 */
export function edgesToBreakingReferences(edges, featureName) {
 ...
 */
export function findDependentFeatures(manifest, allManifests, graph) {
    const ownedFiles = getOwnedFilesFromGraph(manifest, graph);
    const ownedSet = new Set(ownedFiles);
    const dependentFeatures = new Set();
    ...
    for (const otherManifest of allManifests) {
        if (otherManifest.feature === manifest.feature)
            continue;
 ...
 */
export function planRemoval(manifest, allManifests, graph) {
 ...
 */
export function formatRemovalPlan(plan) {
    const lines = [];
 ...
 */
export function generateLinkReplacement(originalMatch, featureName) {
    return `[REMOVED: ${featureName}] <!-- was: ${originalMatch} -->`;
}
/**
 * Apply removal markers to file content
 */
export function applyRemovalMarkers(content, markers, featureName) {
    let result = content;
    ...
    for (const marker of sortedMarkers) {
        ...
        if (lineIndex >= 0 && lineIndex < lines.length) {
            ...
            if (line) {
                ...
                const linkPattern = new RegExp(`\\[${escapeRegex(marker.linkText)}\\]\\([^)]*${escapeRegex(marker.targetFile.split('/').pop() ?? '')}[^)]*\\)`, 'g');
                const replacement = generateLinkReplacement(`[${marker.linkText}](${marker.targetFile})`, featureName);
 ...
 */
export function validateRemoval(plan) {
    const warnings = [];
    ...
    if (plan.stats.featuresAffected > 0) {
        warnings.push(`${plan.stats.featuresAffected} other feature(s) have dependencies on ${plan.featureName}.`);
    }
    if (plan.stats.referencesMarked > 10) {
        warnings.push(`${plan.stats.referencesMarked} references will be marked as removed. Consider reviewing the impact.`);
    }
    return {
